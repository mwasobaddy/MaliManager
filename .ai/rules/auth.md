...
Post-login and post-onboarding redirects go through App\Support\AuthLanding::for(): un-onboarded → onboarding.show; org owners → their tenant subdomain (/properties picker, or /properties/create when org has no properties); occupants → central dashboard. Onboarding.complete and OTP/socialite verify all use it. SESSION_DOMAIN=.malimanager.test shares the session across central + tenant subdomains.
