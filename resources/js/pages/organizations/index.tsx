                    ...
                    <CardContent className="p-4">
                        <form
                            onSubmit={(event) => {
                                ...
                                form.get(orgRoutes.index().url, { preserveState: true, replace: true });
                            }}
                                ...
                                <tbody>
                                    ...
                                    {organizations.data.map((organization) => (
                                        <tr key={organization.id} className="border-b">
                                            ...
                                            <td className="px-3 py-2">{organization.plan ?? '—'}</td>
                                            <td className="px-3 py-2 text-xs">{organization.domain ?? '—'}</td>
                                            <td className="px-3 py-2">{organization.users_count}</td>
                                            <td className="px-3 py-2">
                                                <span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${statusStyles[organization.status] ?? ''}`}>
                                                    {organization.status}
                                                </span>
                                            </td>
                                                ...
                                                <div className="flex justify-end gap-1">
                                                    {canEdit && (
                                                        <Button variant="ghost" size="sm" asChild>
                                                            <Link href={orgRoutes.edit(organization.id)}>
                ...
                {organizations.last_page > 1 && (
                    <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">
                            Page {organizations.current_page} of {organizations.last_page} ({organizations.total} organizations)
                        </span>
                        <div className="flex gap-2">
                            <Button
                                variant="outline"
                                ...
                                onClick={() => router.get(orgRoutes.index().url, { ...form.data, page: organizations.current_page - 1 }, { preserveState: true })}
                            >
                                Previous
                            </Button>
                            <Button
