...
export default function ConversationHistory({ conversations, scope, backUrl, backLabel, resumeUrl, newChatUrl }: Props) {
                ...
                {conversations.length === 0 ? (
                    <Card>
                        <CardContent className="flex flex-col items-center justify-center py-12">
                            ...
                            <Button asChild variant="link" className="mt-2">
                                <Link href={newChatUrl}>Start your first chat</Link>
                            </Button>
                        </CardContent>
                    </Card>
                ) : (
                    <div className="space-y-2">
                        {conversations.map((conversation) => (
                            <Card key={conversation.id} className="transition-colors hover:bg-accent/50">
                                <CardHeader className="pb-2">
                                    <div className="flex items-start justify-between gap-3">
                                        ...
                                        <div className="flex items-center gap-1">
                                            <Button asChild variant="ghost" size="sm">
                                                <Link href={resumeUrl(conversation.id)}>Resume</Link>
                                            </Button>
                                            <Button
                                                variant="ghost"
                                                size="icon"
