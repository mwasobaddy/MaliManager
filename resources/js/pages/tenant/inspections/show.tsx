...
export default function InspectionShow({ propertySlug, inspection }: Props) {
    const generate = () =>
        router.post(
            reportRoute({ property: propertySlug, inspection: inspection.id }).url,
            {},
            { preserveScroll: true },
        );
        ...
        <>
            ...
            <div className="mx-auto max-w-4xl p-8 print:p-0">
                <div className="mb-6 flex flex-wrap items-center justify-between gap-2 print:hidden">
                    ...
                    <div className="flex gap-2">
                        {!inspection.report_generated_at && (
                            <Button onClick={generate} data-test="generate-report-button">
                        ...
                        <Button
                            ...
                            onClick={() =>
                                ...
                                router.delete(
                                    destroyInspection({ property: propertySlug, inspection: inspection.id }).url,
                                    { preserveScroll: true },
                                )
                            }
                        >
                ...
                <header className="mb-6 border-b pb-3">
                    <h2 className="text-xl font-semibold capitalize">{inspection.unit_name} inspection</h2>
                    <p className="text-sm text-muted-foreground">{inspection.inspection_date}</p>
                    {inspection.notes && (
                        <p className="mt-2 text-sm">Inspector notes: {inspection.notes}</p>
                    )}
                </header>
                ...
                {inspection.ai_report ? (
                    <section className="space-y-5">
                        <div>
                            ...
                            <span
                                className={`mt-1 inline-block rounded px-2 py-0.5 text-sm font-medium capitalize ${
                                    conditionStyles[inspection.ai_report.overall_condition] ?? ''
                                }`}
                            >
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                {(inspection.ai_report.areas ?? []).map((area, index) => (
                                    <div key={index} className="rounded-lg border border-input p-3 break-inside-avoid">
                                        <div className="flex items-center justify-between gap-2">
                                            <span className="font-medium capitalize">{area.area}</span>
                                            <span className={`rounded px-1.5 py-0.5 text-xs capitalize ${conditionStyles[area.condition] ?? ''}`}>
                                                {area.condition}
                                            </span>
                                        </div>
                                        <p className="mt-1 text-sm text-muted-foreground">{area.issues}</p>
                                    </div>
                                ))}
                            </div>
                        </div>

                        {(inspection.ai_report.recommendations?.length ?? 0) > 0 && (
                            <div>
                                <h3 className="mb-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
                                    Recommendations
                                </h3>
                                <ul className="list-disc space-y-1 pl-5 text-sm">
                                    {inspection.ai_report.recommendations.map((recommendation, index) => (
                    ...
                    <p className="text-sm text-muted-foreground">
                        No AI report generated yet{inspection.photos.length === 0 ? ' — add photos first.' : '.'}
                    </p>
                )}
            </div>
        </>
