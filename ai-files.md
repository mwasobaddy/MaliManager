...
copy-only. *(Fix: this file was missing `use Inertia\Response;`, which crashed
`page()` — now added.)*
...
try/catch → toast. *(Note: `InspectionController` also defines a duplicate
`reportSchema()` that is currently unused dead code.)*
...
cards; the design intends the assistant to *explain* them.
...
`.catch` for non-JSON failures. *(Fix: previously always rendered an error
